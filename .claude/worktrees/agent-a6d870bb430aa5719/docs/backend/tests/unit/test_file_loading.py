...
def test_plain_text_file_loads_as_unstructured_text(sample_txt_file):
    ...
    """
    document = load_file(sample_txt_file)

    assert document.headings is None
    assert document.content == sample_txt_file.read_text(encoding="utf-8")
...
def test_pdf_with_clean_headings_preserves_heading_structure(sample_pdf_file):
    ...
    """
    document = load_file(sample_pdf_file)

    assert document.headings is not None
    assert document.headings == ["## Machine Learning", "## Artificial Intelligence"]


def test_pdf_without_headings_loads_as_flat_text(sample_pdf_file_no_headings):
    """
    ...
    """
    document = load_file(sample_pdf_file_no_headings)

    assert document.headings is None
    assert "first paragraph" in document.content
    assert "second paragraph" in document.content
    assert "third and final paragraph" in document.content
