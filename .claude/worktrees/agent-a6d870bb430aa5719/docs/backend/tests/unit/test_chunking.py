...
def test_plain_text_without_headings_falls_back_to_paragraph_splitting(sample_txt_file):
    ...
    assert "Cellular respiration" in chunks[1].text
    assert "Both processes" in chunks[2].text


def test_pdf_without_headings_falls_back_to_paragraph_splitting(sample_pdf_file_no_headings):
    """
    ...
    assert "third and final paragraph" in chunks[2].text
