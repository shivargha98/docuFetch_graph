...
def test_mixed_format_folder_ingests_correctly(
    tmp_watch_folder, sample_markdown_file, sample_txt_file, sample_pdf_file, mock_extraction_llm, tmp_path, monkeypatch
):
    """
    ...
    docx_file = tmp_watch_folder / "notes.docx"
    docx_file.write_text("ignored", encoding="utf-8")
    ...
    md_result = pipeline.ingest_file(sample_markdown_file, graph_store)
    txt_result = pipeline.ingest_file(sample_txt_file, graph_store)
    pdf_result = pipeline.ingest_file(sample_pdf_file, graph_store)
    docx_result = pipeline.ingest_file(docx_file, graph_store)

    assert md_result.chunk_count == 2
    assert pdf_result.chunk_count == 2
    assert txt_result.chunk_count == 3

    assert docx_result.skipped is True
    assert docx_result.chunk_count == 0


def test_ingested_chunks_embedded_and_traceable_to_graph_nodes_end_to_end(
    sample_markdown_file, mock_extraction_llm, mock_embedding_client, chroma_test_client, tmp_path, monkeypatch
):
    """
    Given a freshly ingested file producing both graph nodes and Chroma
    embeddings,
    when the full ingest run completes,
    a query embedding should retrieve the expected chunks from Chroma, and
    each retrieved chunk should resolve back to graph node id(s) that the
    same ingest run produced (schema finalized: "graph_node_ids" metadata
    field, see backend_context.md decision #1).
    ...
    graph_store = GraphStore()
    result = pipeline.ingest_file(sample_markdown_file, graph_store, vector_store=chroma_test_client)

    assert result.chunk_count == 2
    assert chroma_test_client.collection.count() == 2

    query_results = chroma_test_client.query("machine learning and artificial intelligence", top_k=2)
    assert len(query_results) == 2

    retrieved_node_ids = set()
    for match in query_results:
        assert match["source_file"] == str(sample_markdown_file)
        retrieved_node_ids.update(match["graph_node_ids"])

    assert retrieved_node_ids == set(result.node_ids)
