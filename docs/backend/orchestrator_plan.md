...
## Process amendment (post-Round 2): shared-file reconciliation step
Round 2 revealed that parallel workers in isolated worktrees copy shared files (`docs/backend/tests/conftest.py`, `requirements.txt`, `backend_context.md`) in and out wholesale rather than merging via git (no worker commits, so no git-mergeable history exists). When 2+ parallel workers in the same round need to add to the same shared file, the last one to write back silently discards the others' edits to it. Full diagnosis and recovery in `backend_context.md` "Round 2 shared-file race" and `backend_TASKS.md`'s log.

**Amended process from Round 3 onward:** whenever a round's parallel workers will each need to add something to `conftest.py` (or another shared file), the workers still run in parallel for their own exclusively-owned production files, but a dedicated serial reconciliation worker runs immediately after to add all of that round's needed shared-file fixtures/entries in one pass — collected from each worker's report rather than trusting parallel write-backs to the same file.

**Amendment #2 (post-Round 3):** the same hazard extends to shared *production* files, not just test infra. Round 3 assigned both parallel workers to add a different function to the same `backend/clients/openrouter_client.py` — one worker's addition was silently reverted by the other's later whole-file write-back before being caught and self-corrected. From Round 4 onward: when writing briefs, explicitly check whether 2+ parallel workers in the same round need to edit ANY same file (not just the known test-infra list) — if so, either (a) serialize those two workers instead of parallelizing them, or (b) apply the same verify-locally/report-verbatim/merge-serially discipline used for conftest.py.
