...
class VectorStore:
    ...
    def __init__(self, path: str | None = None):
        ...
        self._client = chromadb.PersistentClient(path=path if path is not None else CHROMA_DB_PATH)
        self.collection = self._client.get_or_create_collection(_COLLECTION_NAME)
    ...
    def clear_all(self) -> None:
        """
        Purge every stored embedding, resetting the collection to empty.

        Used when switching the watched folder to a new path: since the
        collection lives at a fixed on-disk path/name regardless of which
        folder is currently watched, the previous folder's embeddings would
        otherwise remain queryable indefinitely alongside the new folder's
        data.
        """
